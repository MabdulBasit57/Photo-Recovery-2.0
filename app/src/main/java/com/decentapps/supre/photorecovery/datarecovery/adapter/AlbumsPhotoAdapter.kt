...
class AlbumsPhotoAdapter(
    var context: Context,
    var al_menu: ArrayList<AlbumPhoto>,
    var mOnClickItemListener: OnClickItemListener
) : RecyclerView.Adapter<AlbumsPhotoAdapter.MyViewHolder>() {
    ...
    override fun onBindViewHolder(myViewHolder: MyViewHolder, i: Int) {
        ...
        textView.text = al_menu[i].listPhoto?.size.toString() + " Pictures"
        val sectionListPhotoDataAdapter = al_menu[i].listPhoto?.let {
            SectionListPhotoDataAdapter(
                this.context,
                it, i
            )
        }
